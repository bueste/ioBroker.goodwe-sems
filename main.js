...
class GoodweSems extends utils.Adapter {
    ...
    async _pollCycle() {
        ...
        try {
            ...
            this.log.debug(`Poll-Zyklus erfolgreich (${Date.now() - startedAt} ms), nächster Abruf in ${this.basePollIntervalSec}s.`);
            this._schedulePoll(this.basePollIntervalSec * 1000);
        } catch (error) {
    ...
    async _ensureChannel(id, name) {
        if (this.knownObjectIds.has(`channel:${id}`)) return;
        await this.setObjectNotExistsAsync(id, {
            type: "channel",
            common: { name },
            native: {},
    ...
    async _handlePollError(error) {
        ...
        if (error instanceof SemsRateLimitError) {
            ...
            nextDelaySec = Math.min(this.basePollIntervalSec * Math.pow(2, Math.min(this.consecutiveErrors, 5)), MAX_BACKOFF_SEC);
            await this.notifier.notify(
                "loginFailure",
            ...
            await this.setStateAsync("info.rateLimited", false, true);
            nextDelaySec = Math.min(this.basePollIntervalSec * Math.pow(1.5, Math.min(this.consecutiveErrors, 6)), MAX_BACKOFF_SEC / 2);
        } else {
        ...
        if (this.consecutiveErrors >= this.maxConsecutiveErrors && downMs >= this.stationOfflineMs && !this.stationOfflineNotified) {
    ...
    _maskAccount(account) {
        if (!account) return "(nicht gesetzt)";
