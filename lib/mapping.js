...
const NUM = (unit, role = "value") => ({ type: "number", role, unit, read: true, write: false });
const STR = (role = "text") => ({ type: "string", role, read: true, write: false });
const BOOL = (role = "indicator") => ({ type: "boolean", role, read: true, write: false });
...
function mapMonitorDetail(detail) {
    ...
    push("Station.PortalTimestamp", "Last update time reported by portal", NUM("", "value.time"), parsePortalTimestamp(pick(info, ["time"])));
    ...
    push("PowerFlow.Battery", "Battery charge/discharge power", NUM("W", "value.power"), pickNumber(powerflow, ["bettery", "battery"]));
    ...
    push("PowerFlow.BatteryStatus", "Battery flow direction code", NUM(), pickNumber(powerflow, ["betteryStatus", "batteryStatus"]));
    ...
    if (evCharge || detail.isEvCharge) {
        push("EVCharger.Present", "EV charger present according to portal", BOOL(), Boolean(detail.isEvCharge || evCharge));
    ...
    inverters.forEach((inv, index) => {
        const sn = pick(inv, ["sn", "SN", "invertersn"]) || `UNKNOWN_${index + 1}`;
...
module.exports = { mapMonitorDetail, pick, pickNumber, parsePortalTimestamp, toNumber };
