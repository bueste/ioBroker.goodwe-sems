...
function mapMonitorDetail(detail) {
    ...
    push("Station.Latitude", "Latitude", NUM("°", "value.gps.latitude"), pickNumber(info, ["latitude"]));
    push("Station.Longitude", "Longitude", NUM("°", "value.gps.longitude"), pickNumber(info, ["longitude"]));
    push(
    ...
    inverters.forEach((rawInv, index) => {
        ...
        // Battery data: only present when the "enableBattery" adapter option is on AND this
        // inverter has an attached battery system (see semsApi.js's _getMonitorDetailGateway()
        // for how inv.battery is populated - relatedDevices()/telemetry() against a separate
        // "BAT_SYS" device, reverse-engineered from real portal traffic). undefined/missing
        // fields are silently skipped by push() as usual, so a battery-less inverter or a
        // disabled option simply produces no Battery.* states, same as before.
        const battery = inv.battery;
        push(
            `${base}.Battery.SOC`,
            "Battery state of charge (this inverter)",
            NUM("%", "value.battery"),
            pickNumber(battery, ["soc"]),
        );
        push(
            `${base}.Battery.Power`,
            "Battery charge/discharge power (this inverter)",
            NUM("kW", "value.power"),
            pickNumber(battery, ["power"]),
        );
        push(`${base}.Battery.Voltage`, "Battery voltage", NUM("V", "value.voltage"), pickNumber(battery, ["voltage"]));
        push(`${base}.Battery.Current`, "Battery current", NUM("A", "value.current"), pickNumber(battery, ["current"]));
        push(
            `${base}.Battery.Temperature`,
            "Battery temperature",
            NUM("\u00b0C", "value.temperature"),
            pickNumber(battery, ["temperature"]),
        );
        push(
            `${base}.Battery.MaxChargeCurrent`,
            "Maximum charge current limit",
            NUM("A", "value.current"),
            pickNumber(battery, ["maxChargeCurrent"]),
        );
        push(
            `${base}.Battery.MaxDischargeCurrent`,
            "Maximum discharge current limit",
            NUM("A", "value.current"),
            pickNumber(battery, ["maxDischargeCurrent"]),
        );
    });
