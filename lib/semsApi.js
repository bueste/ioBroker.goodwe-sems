...
const DEFAULT_CLIENT_TOKEN = JSON.stringify({ version: "v3.1", client: "ios", language: "en" });
...
class SemsApi {
    ...
    constructor({ account, password, requestTimeoutMs = 15000, log }) {
        ...
        /** @type {{uid:string, token:string, timestamp:number, api:string}|null} */
        this.session = null;
    }
    ...
    async _loginLegacy() {
        const body = JSON.stringify({ account: this.account, pwd: this.password });
        ...
        return this._extractSession(json, LEGACY_FALLBACK_API, "Legacy");
    }
