...
// under different API versions depending on which login variant the account
// ends up using (confirmed against a real account whose SEMS+ login is
// rejected with code=C0602, falling back to the legacy CrossLogin API): the
// "v3" path works for SEMS+ sessions, but 404s ("Route Not Found") on a
// legacy-login session, which instead needs "v2" - matching the community
// reference implementation pygoodwe (github.com/yaleman/pygoodwe), whose
// legacy-only client hardcodes "v2/PowerStation/GetMonitorDetailByPowerstationId".
const MONITOR_DETAIL_PATH_V3 = "/v3/PowerStation/GetMonitorDetailByPowerstationId";
const MONITOR_DETAIL_PATH_V2 = "/v2/PowerStation/GetMonitorDetailByPowerstationId";
...
class SemsApi {
    ...
    async getMonitorDetail(powerStationId) {
        ...
        try {
            return await this._authenticatedPost(MONITOR_DETAIL_PATH_V3, { powerStationId });
        } catch (error) {
            if (error instanceof SemsProtocolError && /404|route not found/i.test(error.message)) {
                this.log(
                    "debug",
                    `${MONITOR_DETAIL_PATH_V3} nicht gefunden (404) - versuche ${MONITOR_DETAIL_PATH_V2} als Fallback (vermutlich Legacy-API-Backend).`,
                );
                return this._authenticatedPost(MONITOR_DETAIL_PATH_V2, { powerStationId });
            }
            throw error;
        }
    }
