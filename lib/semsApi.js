...
async function httpPostJson(url, init, timerFns) {
    ...
    try {
        ...
        try {
            ...
            throw new SemsProtocolError(`Response from ${url} was not valid JSON: ${parseError.message}`);
        }
        return { status: response.status, json, text };
    } catch (error) {
        if (error.name === "AbortError") {
            throw new SemsNetworkError(`Timeout after ${timeoutMs} ms at ${url}`);
        }
        if (error instanceof SemsProtocolError) {
            throw error;
        }
        throw new SemsNetworkError(`Network error at ${url}: ${error.message}`);
    } finally {
...
class SemsApi {
     ...
     * @param {boolean} [opts.enableBattery] opt-in: also fetch per-inverter battery telemetry via
     *   GoodWe's undocumented web-portal API (relatedDevices()/telemetry() against a "BAT_SYS"
     *   device). Off by default - see _getMonitorDetailGateway() for details and caveats.
     */
    constructor({
        account,
        password,
        ...
        if (!account || !password) {
            throw new SemsAuthError("SEMS credentials (username/password) are missing in the adapter configuration.");
        }
    ...
    _authHeaders() {
        if (!this.session) {
            throw new SemsAuthError("No active SEMS session - login() was not called (successfully).");
        }
    ...
    async login() {
        try {
            ...
            this.log("debug", `SEMS login via SEMS+ (new API) successful. API base: ${session.api}`);
            this.session = session;
            return session;
        } catch (newLoginError) {
            this.log("debug", `SEMS+ login failed (${newLoginError.message}), falling back to legacy login.`);
        }

        const session = await this._loginLegacy();
        this.log("debug", `SEMS login via legacy API successful. API base: ${session.api}`);
        this.session = session;
        return session;
    }
    ...
    _extractSession(json, fallbackApi, variantName) {
        ...
        if (!SUCCESS_CODES.has(code) && !(json && SUCCESS_MESSAGES.has(String(json.msg).toLowerCase()))) {
            const msg = (json && (json.msg || json.message || json.error_msg)) || "unknown error";
            throw new SemsAuthError(`SEMS ${variantName} login rejected: ${msg} (code=${code})`);
        }

        const data = json && json.data;
        if (!data || !data.token || !data.uid) {
            throw new SemsProtocolError(
                `SEMS ${variantName} login returned no usable session data (response keys: ${Object.keys(json || {}).join(", ")}).`,
            );
        }
    ...
    _validateApiBase(candidate, fallbackApi) {
        ...
        this.log(
            "warn",
            `SEMS login returned an unexpected API base URL ("${candidate}") - using ${fallbackApi} instead.`,
        );
        return fallbackApi;
    }
    ...
    async _authenticatedPost(path, payload, isRetry = false) {
        ...
        this.log("debug", `SEMS API response ${url}: ${JSON.stringify(json)}`);

        const code = json && json.code;

        if (String(code) === RATE_LIMIT_CODE) {
            throw new SemsRateLimitError(
                `SEMS portal rejected the request with rate-limit code ${RATE_LIMIT_CODE}.`,
        ...
        if (looksExpired && !isRetry) {
            this.log("debug", "SEMS session expired, refreshing token and retrying the request once.");
            this.session = null;
            await this.login();
        ...
        if (!SUCCESS_CODES.has(code) && !(json && SUCCESS_MESSAGES.has(String(json.msg).toLowerCase()))) {
            const msg = (json && (json.msg || json.message || json.error_msg)) || "unknown error";
            throw new SemsProtocolError(`SEMS API call ${path} failed: ${msg} (code=${code})`);
        }
    ...
    async getMonitorDetail(powerStationId) {
        if (!powerStationId) {
            throw new SemsProtocolError("getMonitorDetail() called without a powerStationId.");
        }
    ...
    async _gatewayRequest(method, path, query, isRetry = false) {
        ...
        this.log("debug", `SEMS gateway response ${url}: ${JSON.stringify(json)}`);
        ...
        if (!SUCCESS_CODES.has(code)) {
            ...
            if (!isRetry) {
                this.log(
                    "debug",
                    `SEMS gateway call ${path} returned an error (${msg}, code=${code}) - refreshing session and retrying once.`,
                );
                this.session = null;
                await this.login();
                return this._gatewayRequest(method, path, query, true);
            }
            throw new SemsProtocolError(`SEMS gateway call ${path} failed: ${msg} (code=${code})`);
        }
        return json.data;
    }
    ...
    async _getMonitorDetailGateway(powerStationId) {
        ...
        try {
            ...
            this.log("debug", `SEMS gateway: device list unavailable (${error.message}).`);
        }
        ...
        for (const device of deviceList) {
            ...
            try {
                ...
                this.log("debug", `SEMS gateway: telemetry for ${sn} failed (${error.message}).`);
            }
            try {
                ...
                this.log("debug", `SEMS gateway: telecounting for ${sn} failed (${error.message}).`);
            }

            const pacKw = gatewayNum(telemetryFlat.pAc);

            // Battery telemetry (opt-in, see the "enableBattery" adapter config option). Uses the
            // same relatedDevices()->per-device telemetry() pattern as GoodWe's own web portal:
            // a station's device-list endpoint (stations/device/all-status, used above for the
            // inverter list itself) does NOT include attached battery systems - they only surface
            // via a SEPARATE per-inverter relatedDevices() call, which returns each attached
            // device's own ID (a different ID format, "VD..." prefixed, not the inverter serial)
            // and deviceType (confirmed "BAT_SYS" for a GoodWe LX-series battery). Reverse-engineered
            // and verified byte-for-byte against real captured browser traffic (HAR), incl. the
            // gateway's SHA-256 request-signature scheme (see _gatewaySignature) - there is no
            // documented API for this. Deliberately opt-in and fully isolated in try/catch: this
            // undocumented flow could change or fail at any time without affecting core monitoring.
            let battery;
            if (this.enableBattery) {
                try {
                    const related = await this._gatewayRequest(
                        "GET",
                        `/sems-plant/api/equipments/${encodeURIComponent(sn)}/relatedDevices`,
                        { sn, deviceType, pwId: powerStationId },
                    );
                    const batterySn = Array.isArray(related)
                        ? related.find(d => d && d.type === "BAT_SYS" && d.sn)
                        : undefined;
                    if (batterySn && batterySn.sn) {
                        const batteryTelemetry = await this._gatewayRequest(
                            "GET",
                            `/sems-plant/api/equipments/${encodeURIComponent(batterySn.sn)}/telemetry`,
                            { deviceType: "BAT_SYS", pwId: powerStationId },
                        );
                        const batteryFlat = flattenGatewayFactors(batteryTelemetry);
                        battery = {
                            soc: gatewayNum(batteryFlat.soc),
                            power: gatewayNum(batteryFlat.pBat),
                            voltage: gatewayNum(batteryFlat.voltage),
                            current: gatewayNum(batteryFlat.a),
                            temperature: gatewayNum(batteryFlat.batSysTemp),
                            maxChargeCurrent: gatewayNum(batteryFlat.aMaxChar),
                            maxDischargeCurrent: gatewayNum(batteryFlat.aMaxDischar),
                        };
                    }
                } catch (error) {
                    this.log("debug", `SEMS gateway: battery telemetry for ${sn} unavailable (${error.message}).`);
                }
            }
